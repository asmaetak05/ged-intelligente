  ...
  return (
    <div className="p-8 h-full flex flex-col gap-6">
      <div className="flex items-center gap-2 pb-4 border-b border-zinc-200">
        <Server className="text-zinc-900" />
        <h1 className="text-2xl font-semibold text-zinc-900 tracking-tight">Pipeline & Admin</h1>
      </div>
      ...
      <div className="flex flex-1 gap-6 min-h-0">
        ...
        <div className="w-1/2 flex flex-col gap-6 overflow-y-auto pr-2">
          ...
          <div className="bg-white p-5 rounded-xl border border-zinc-200 shadow-sm">
            <h2 className="text-lg font-medium mb-4 text-zinc-900">Contrôleur de Pipeline</h2>
            
            <div className="flex gap-4 mb-6">
              <div className="flex-1">
                <label className="block text-xs font-medium text-zinc-500 mb-1">Date de début (Scraping)</label>
                <input 
                  type="text" 
                  value={dates.start}
                  onChange={(e) => setDates({...dates, start: e.target.value})}
                  className="w-full border border-zinc-300 rounded-md px-3 py-2 text-sm focus:outline-none focus:border-zinc-500" 
                  placeholder="JJ/MM/AAAA"
                />
              </div>
              <div className="flex-1">
                <label className="block text-xs font-medium text-zinc-500 mb-1">Date de fin (Scraping)</label>
                <input 
                  type="text" 
                  value={dates.end}
            ...
            <div className="flex flex-col gap-3">
              <button 
                ...
                className="flex items-center justify-center gap-2 w-full bg-zinc-900 hover:bg-zinc-800 text-white py-2.5 rounded-md text-sm font-medium transition-colors">
                <Play size={16} /> Lancer le Scraping (Téléchargement)
              </button>
              <button 
                onClick={() => sendCommand('extract')}
                className="flex items-center justify-center gap-2 w-full bg-white hover:bg-zinc-50 border border-zinc-300 text-zinc-700 py-2.5 rounded-md text-sm font-medium transition-colors">
          ...
          <div className="bg-white p-5 rounded-xl border border-zinc-200 shadow-sm flex-1">
            <h2 className="text-lg font-medium mb-4 text-zinc-900">Schéma de la Base</h2>
            <div className="space-y-4">
              {schema.map((table, idx) => (
                <div key={idx} className="border border-zinc-100 rounded-lg overflow-hidden">
                  <div className="bg-zinc-50 px-3 py-2 border-b border-zinc-100 font-medium text-sm text-zinc-800 flex justify-between">
                    <span>{table.table}</span>
                    <span className="text-xs text-zinc-500">{table.columns.length} cols</span>
                  </div>
                  <div className="p-3 text-xs bg-white text-zinc-600 max-h-40 overflow-y-auto">
                    <ul className="space-y-1">
                      {table.columns.map((col, cidx) => (
                        <li key={cidx} className="flex justify-between">
                          <span className={col.primary_key ? 'font-bold text-zinc-900' : ''}>{col.name}</span>
